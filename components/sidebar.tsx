...
// Cachear 5 min: esta query corría en CADA página para CADA usuario
const getTournamentToday = unstable_cache(
  async () => {
...
const SectionLabel = ({ children }: { children: React.ReactNode }) => (
  <p className="px-4 pt-4 pb-1 text-[11px] font-bold text-muted-foreground/70 uppercase tracking-widest">
    {children}
  </p>
);
...
export const Sidebar = async ({ className }: Props) => {
  ...
  try {
    hasTournamentToday = await getTournamentToday();
  } catch {
    // Si la BD no responde, el sidebar no debe romper la página
  }
  ...
  return (
    <div className={cn(
      ...
      <Link href="/learn">
        <div className="pt-8 pl-4 pb-5 flex items-center gap-x-3 group">
          <div className="relative animate-float">
            <Image src="/mascot.svg" height={40} width={40} alt="Mascot" className="drop-shadow-[0_0_15px_rgba(16,185,129,0.5)]" />
          </div>
          <h1 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-teal-400 tracking-wide group-hover:from-emerald-400 group-hover:to-teal-300 transition-all">
            RutaCachimbo
          </h1>
        </div>
      </Link>
      <div className="flex flex-col gap-y-1 flex-1 overflow-y-auto custom-scrollbar pb-6 pr-2">
        ...
        <SectionLabel>Competir</SectionLabel>
        <SidebarItem
          label="Clasificación"
          href="/leaderboard"
